...
const OFFLINE_WORK = { DOI: "10.48550/arXiv.2307.08724", title: ["A Mathematical Framework for Transformer Circuits"], "container-title": ["arXiv"], issued: { "date-parts": [[2023, 7, 17]] }, type: "posted-content", URL: "https://doi.org/10.48550/arXiv.2307.08724", author: [{ given: "Nelson", family: "Elhage" }], "is-referenced-by-count": 100 };
function firstString(value) {
    if (Array.isArray(value))
        return value.map(normalizeText).find(Boolean) ?? null;
    return normalizeText(value) || null;
}
function mapMessage(message, query, score) {
...
function crossrefUrl(input, doi) {
    ...
    if (!doi) {
        ...
        if (input.dateRange)
            url.searchParams.set("filter", `from-pub-date:${input.dateRange.from},until-pub-date:${input.dateRange.to}`);
    }
    if (input.runtime.contactEmail)
        url.searchParams.set("mailto", input.runtime.contactEmail);
    return url;
}
export function buildCrossrefSearchUrl(input) {
...
export async function searchCrossref(input) {
    ...
    if (input.offline)
        return (doi || query.toLowerCase() === "a mathematical framework for transformer circuits" ? [OFFLINE_WORK] : []).slice(0, input.limit).map((item) => mapMessage(item, query, 1)).filter((item) => Boolean(item));
    ...
    const messages = doi ? [record(payload?.message)] : (Array.isArray(items) ? items : []);
    return messages.map((item, index) => mapMessage(record(item) ?? {}, query, doi ? 1 : Math.max(0.1, 1 - index * 0.03))).filter((item) => Boolean(item));
}
