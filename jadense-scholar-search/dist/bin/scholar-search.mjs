...
function valueFor(argv, index, flag) { const value = argv[index + 1]; if (!value || value.startsWith("-"))
    throw new Error(`${flag} requires a value.`); return value; }
function splitValues(value) { return value.split(",").map((item) => item.trim()).filter(Boolean); }
function provider(value) { if (!SEARCH_PROVIDER_IDS.includes(value))
    throw new Error(`Invalid provider: ${value}. Expected one of ${SEARCH_PROVIDER_IDS.join(", ")}.`); return value; }
function parseArgs(argv) {
    ...
    let offline = false;
    let format = "json";
    let userAgent = process.env.SCHOLAR_SEARCH_USER_AGENT?.trim() || "scholar-search-cli/0.1.0";
    let contactEmail = process.env.SCHOLAR_SEARCH_CONTACT_EMAIL?.trim() || process.env.CONTACT_EMAIL?.trim() || undefined;
    for (let index = 0; index < argv.length; index += 1) {
        ...
        const read = () => inline ?? valueFor(argv, index++, flag);
        if (flag === "--query" || flag === "-q")
            queries.push(...splitValues(read()));
        else if (flag === "--provider" || flag === "-p")
            selected.push(...splitValues(read()).map(provider));
        else if (flag === "--limit")
            limit = Number(read());
        else if (flag === "--from")
            from = read();
    ...
    const dateRange = from || to ? { from: from ?? "", to: to ?? "" } : undefined;
...
function markdown(response) {
    ...
    if (response.diagnostics.length) {
        ...
        response.diagnostics.forEach((item) => lines.push(`- [${item.severity}] ${item.provider ?? item.stage}: ${item.message}`));
    }
    return `${lines.join("\n")}\n`;
}
...
export async function runCli(argv = process.argv.slice(2)) {
    try {
        ...
        const code = error instanceof ScholarSearchError ? error.code : "invalid_request";
        const message = error instanceof Error ? error.message : "Search failed.";
        const payload = { error: { code, message, ...(error instanceof ScholarSearchError && error.provider ? { provider: error.provider } : {}) } };
...
//# sourceMappingURL=scholar-search.mjs.map