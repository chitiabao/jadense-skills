...
export async function searchGoogleScholar(input: ProviderSearchInput): Promise<PaperCandidate[]> {
  ...
  const results: PaperCandidate[] = []; const seen = new Set<string>()
  for (let start = 0; start < input.limit; start += 10) {
    let payload: ReturnType<typeof record>
...
export const googleScholarProvider: ProviderAdapter = { id: "google_scholar", search: searchGoogleScholar }
