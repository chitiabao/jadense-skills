...
const OFFLINE_RESULTS: Record<string, Array<Record<string, unknown>>> = {
...
function elements(parent: XmlElement, tagName: string) {
  const nodes = parent.getElementsByTagName(tagName)
  return Array.from({ length: nodes.length }, (_, index) => nodes.item(index)).filter((node): node is XmlElement => Boolean(node))
}
function first(parent: XmlElement, tagName: string) { const nodes = parent.getElementsByTagName(tagName); return nodes.length ? nodes.item(0) : null }
function text(node: XmlElement | null) { return normalizeText(node?.textContent) }
function year(value: string) { const match = value.match(/^(19|20)\d{2}/); return match ? Number(match[0]) : null }

function mapEntry(entry: XmlElement, query: string, index: number): PaperCandidate | null {
...
function mapOffline(value: Record<string, unknown>, query: string, index: number): PaperCandidate | null {
...
export function buildArxivSearchUrl(input: ProviderSearchInput): string {
...
export async function searchArxiv(input: ProviderSearchInput): Promise<PaperCandidate[]> {
  const query = normalizeText(input.query)
  if (!query) return []
  if (input.offline) return (OFFLINE_RESULTS[query.toLowerCase()] ?? []).slice(0, input.limit).map((item, index) => mapOffline(item, query, index)).filter((item): item is PaperCandidate => Boolean(item))
  ...
  return document.documentElement ? elements(document.documentElement, "entry").map((entry, index) => mapEntry(entry, query, index)).filter((item): item is PaperCandidate => Boolean(item)) : []
}

export const arxivProvider: ProviderAdapter = { id: "arxiv", search: searchArxiv, detail: async ({ candidate, runtime, offline }) => { const id = normalizeExternalId("arxiv", candidate.externalId); return id ? (await searchArxiv({ query: `id:${id}`, limit: 1, runtime, offline }))[0] ?? null : null } }
