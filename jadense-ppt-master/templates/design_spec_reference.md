...
除院校/会议模板或用户明确要求另有规定外，学术项目采用以下默认值，并把最终选择写入下方既有结构；这些默认值不改变 `design-spec/v1` schema：

- 画布为 `ppt169`（1280×720，16:9）；只有用户明确要求兼容旧教室或传统投影设备时才用 `ppt43`。
- Style 为 `academic-research`；自动视觉风格只限 `swiss-minimal`、`editorial`、`data-journalism`、`blueprint`，其余商业/艺术风格必须由用户显式选择。
- 图片来源默认为 `none`，有科研证据图片时优先 `provided`；图标默认为 `none`。AI 只可生成明确标注的概念示意，不得伪装成实验或观测证据。
- 默认色为白/暖白背景、深海军蓝结构、冷灰辅助和青蓝数据强调；颜色只编码已声明的变量或状态。默认中文字体为微软雅黑/等线，拉丁字符为 Arial，公式为 Cambria Math。
- 1280×720 下，封面标题约 68px、页标题至少 48px、正文至少 24px、来源/脚注 16–18px；禁止渐变、3D、玻璃拟态、营销式卡片阵列、装饰性满版图和花哨动画。
- 正式性、证据可追溯和院校/会议要求优先于视觉风格；不得虚构论文、数据、样本量、统计显著性或结论。
...
| Report Type | <开题、中期、预答辩、正式答辩、学术会议、组会、项目汇报、基金评审，或用户确认的其他类型> |
| Discipline | <学科、专业与必要的研究方向> |
| Degree / Training Stage | <本科、硕士、博士、博士后、教师/研究人员，及当前培养阶段> |
| Institution / Conference | <院校、院系、实验室、项目或会议；未知时写“未提供”，不得猜测> |
| Academic Audience / Review Panel | <导师、评委、同行、跨学科听众等角色及专业背景> |
| Presentation Duration | <确认的总时长；未提供时记录工作假设> |
| Required Sections | <院校/会议/用户规定的必需章节与顺序；无时写“无额外强制章节”> |
| Citation Style | <GB/T 7714、APA、IEEE、页内作者-年份、编号制或确认的其他格式> |
| Evidence Status | <逐项区分已核验事实、真实数据、复现结果、演示数据、预期结果与待补证据> |
...
九个学术字段属于 §I 的项目级沟通事实，必须填写为已确认值、来源可推导值或明确的“未提供 / 工作假设 / 待补”，不得删除、猜测或用空白占位。`Academic Audience / Review Panel` 记录学术身份与专业程度；既有 `Target Audience` 继续记录沟通所需的行为画像与期待结果，两者不能互相替代。`Evidence Status` 必须说明全稿中真实、复现、演示、预期和待验证材料的标注规则。
