...
本派生 Skill 以 [`academic-research`](./academic-research/templates/design_spec.md) 为默认 Style。只要请求属于学术开题、中期、预答辩、答辩、学术会议、组会、项目汇报或基金评审，且用户没有提供一个精确 Style 根目录，就自动选择 `academic-research`；不得因为“科技”“高端”“发布”等表面词汇改选商业或艺术 Style。

`styles_index.json` 继续登记上游的全部 Style，以保持骨架和显式选择能力。除 `academic-research` 外，其余 Style 只有在用户明确点名该 Style 或明确要求对应的非学术报告类型时才能启用；不能作为学术请求的自动候选。院校、课题组或会议的正式模板与用户明确要求优先于本默认值，但不得放宽证据真实性和来源可追溯要求。
