...
Upstream versions, compatibility overlays, licenses, attribution, and trademark boundaries are recorded in [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md).
...
```bash
python3 ${SKILL_DIR}/scripts/icon_sync.py <project_path> tabler-outline/home tabler-outline/bulb simple-icons/github
```
...
```xml
<!-- chunk-filled (compact, chunky — strong small-size legibility) -->
...
```bash
rg --files "${SKILL_DIR}/templates/icons/tabler-outline" -g '*chart*.svg'
...
- **`chunk-filled`** — **fill** style, designed as compact 16px silhouettes. Bold forms may combine rectilinear and curved geometry while staying highly legible at small sizes. Visual weight: heavy, solid, compact.
...
> 1. **Geometry**: compact silhouettes (`chunk-filled`) vs. rounded curves (`tabler-filled` / `phosphor-duotone`) vs. open strokes (`tabler-outline`)
