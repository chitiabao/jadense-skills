...
**学术提示层（必须）**：进入 Quick 后立即完整读取
[`academic-core.md`](../../references/academic-core.md)，并在本次不中断的活动上下文中
持续应用。当请求或材料属于开题、中期、预答辩、正式答辩、学术会议、组会、项目/
基金评审等场景时，再读取
[`academic-report-types.md`](../../references/academic-report-types.md) 的对应类型。Quick
省略确认和持久规划，不省略学术证据、引用、数据标签、中文排版或正式性要求。
...
**Academic sufficiency**：先按 `academic-core.md` 区分外部事实、用户提供、推导和
演示/情景数据。论文结论、作者/年份/DOI、样本量、统计显著性、图表和科研图像若没有
可定位证据，立即做有界研究或标为待补；Quick 不得以“快速”为由猜测。已触发的报告
类型只决定需要支持哪些论证环节，不授权搜索与本次报告无关的背景材料。
...
**Academic one-pass context**：学术任务同时解析报告类型、学科、培养阶段、院校/会议、
听众、时长、强制章节、引用格式和证据状态，并只保留在当前活动上下文。材料足以判断
时直接继续；无法判断且会实质改变必需章节/证据口径时，采用最保守的通用学术报告
结构并在交付说明中披露假设，不创建 Design Spec 替代物。

除非用户、强制模板或源稿保真要求另有规定，学术 Quick 在活动上下文中默认执行
`academic-research` **Style** 的学术约束；独立的 `visual_style` 轴只能从
`swiss-minimal`、`editorial`、`data-journalism` 或 `blueprint` 中选择最适合学科与
证据载体的一项（或基于这些 id 形成 `custom`），不得把 `academic-research` 写成
`visual_style` id。默认白/暖白底、深海军蓝结构色、冷灰辅助、青蓝数据色，16:9
画布，不用图标或 AI 配图。艺术/营销风格只有显式要求时才可选。
...
- for an academic deck, the page-level citation carrier, evidence-status labels,
  statistical qualifiers, figure/table source line, and—when ordinary speaker
  notes are enabled without narration audio—the per-page `[Sources]` block from
  `academic-core.md`; never invent a citation merely to close this transient state;
...
retain the already-read [`academic-core.md`](../../references/academic-core.md),
...
For academic pages, the transient execution anchors also include the report
type's page duty, evidence class, page-local short citation, demo/scenario
disclosure, and any sample/unit/uncertainty qualifiers. Preserve Chinese
punctuation and line-breaking rules plus the `1280×720` typography floors from
`academic-core.md`; if the content does not fit, use the profile-permitted
restructure/split/shorten path rather than silently shrinking sources or body
...
For an academic deck without Narration Audio, every page note ends with the
complete `[Sources]` block required by `academic-core.md`, including an explicit
user-material/demo/no-external-source entry where applicable. With Narration
Audio, keep the input pure spoken prose and carry complete citations through
page/appendix/source artifacts as defined by that core.
