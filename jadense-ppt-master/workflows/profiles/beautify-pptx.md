...
**学术提示层（必须）**：进入 Beautify 后立即完整读取
[`academic-core.md`](../../references/academic-core.md)。当请求或源稿属于开题、中期、
预答辩、正式答辩、学术会议、组会、项目/基金评审等场景时，再读取
[`academic-report-types.md`](../../references/academic-report-types.md) 的对应类型。该层
约束证据语义、正式性、中文排版和视觉风险；它不削弱本 Profile 的逐字、1:1 与源
身份保真边界。
...
**Academic fidelity**：学术源稿中的题目、作者/机构、图表题注、引用编号、脚注、单位、
样本量、误差定义、显著性符号、公式和“演示/待验证”等状态文字全部属于冻结内容，
不得因排版而省略或改写。源稿缺少来源、统计限定或 AI 示意标签时，只在最终 QA 中
列为待作者核验；Beautify 不得自行补论文、DOI、统计量或结论来“完善”内容。
...
For an academic source, regeneration also preserves axis domain/baseline, units,
series/category order when meaningful, uncertainty encoding, sample qualifiers,
table missing-value symbols, figure/table numbering, short citations, captions,
and source lines. Never replace a real research figure with AI artwork or turn a
flat scientific chart into 3D. The original picture remains evidence-bearing;
crop only where no label, scale bar, subject identity, annotation, or context is
lost.
...
When the source is academic, keep the report type and every evidence-state
marker in transient context. Apply the formal visual defaults from
`academic-core.md` only to choices the user/source identity leaves open: source
palette/fonts and explicit template rules remain truth, while decorative icons,
AI images, gradients, 3D, marketing cards, and ornamental motion are never added
by default.
...
For academic decks, the confirmation recommendation also surfaces any body,
source-line, or annotation size below `academic-core.md`'s projection-readable
floors, and any source design that relies on 3D/distorted axes, color alone, or
decorative imagery around evidence. Because content and source identity remain
profile constraints, recommend a compatible correction or disclose the risk;
do not silently rewrite content, fabricate labels, or switch identity.
...
When academic speaker notes are enabled without Narration Audio, end every page
note with `academic-core.md`'s `[Sources]` block. Source-PPT material may be
recorded as `User-supplied: <stem>.pptx, slide <N>`; retain any fuller citation
already present on that source slide. Do not infer a missing bibliographic entry
from a title fragment. With Narration Audio, follow the core's pure-spoken-input
boundary.
...
| Academic evidence | citations, units, uncertainty, formula text, figure/table labels, demo-state wording, and evidence images retain their source meaning |
| Academic QA gaps | missing/ambiguous citation, significance, AI provenance, or tiny-source risks are reported, never silently invented or repaired through copy changes |
