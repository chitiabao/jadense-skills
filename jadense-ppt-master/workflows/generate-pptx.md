...
**学术提示层（必须）**：进入本路由后立即完整读取
[`academic-core.md`](../references/academic-core.md)，并在整个 Step 1–7 中持续应用。
当请求或材料触发开题、中期、预答辩、正式答辩、学术会议、组会、项目/基金评审或
其他明确学术报告时，再完整读取
[`academic-report-types.md`](../references/academic-report-types.md) 中对应类型；普通非学术
任务不加载类型规范。该提示层只约束内容、证据与学术设计，不改变本文件的路由、Gate、
CLI、schema 或导出顺序。
...
**学术材料补充检查**：按 `academic-core.md` 先区分来源事实、用户材料、推导与演示/
情景数据，再判断充分性。论文、统计结论、图表、科研图像和政策/标准需要可定位证据；
缺失时进入 `topic-research` 或明确标为待补，不得由 Strategist/Executor 补写一个貌似
合理的作者、DOI、样本量、结果或显著性。报告类型已触发时，用对应类型规范确定真正
需要补证的论证环节，不为凑齐通用章节扩张研究范围。
...
For an academic request, `free_design` means “no user-selected Brand, Layout,
Deck, or external template”; it does **not** disable the bundled
`academic-research` Style baseline. Do not read or install that Style during
Stage 1. When the user supplied no exact root and made no explicit Style or
template request, the post-confirmation handoff below installs it
deterministically before Stage 2. The user can still explicitly select another
Style/template, in which case the deterministic default is not added.
...
```
Retain references/academic-core.md              # REQUIRED: already read at route entry
...
|---|---|
| `academic-core.md` identifies an academic report type | `references/academic-report-types.md` for that report type; retain it through §IX authoring |
| Stage 1 is confirmed and either its template choice or the deterministic academic `academic-research` baseline installed a Brand/Style/Layout/Deck workspace into this project | `references/strategist-template.md` before Stage 2 |
...
**Academic Project Information**：学术任务在不新增确认阶段的前提下，解析报告类型、
学科、培养阶段、院校/会议、听众、时长、强制章节、引用格式与证据状态。能从请求、
材料或强制模板确定的直接使用；会实质改变章节、证据口径或时长分配且无法确定的，
Default 将这些缺口合并到一次既有澄清中。最终值写入 `design_spec.md §I`，并由 §IX
按对应报告类型的论证职责实现；不得把这些字段伪造成 Confirm UI 新 schema。
...
1. For `templates`, load and run
   ...
   For `free_design`, normally skip installation. However, when the request is
   academic, the user supplied no exact root, and the user made no explicit
   Style/template request, run the same apply stage once against
   `${SKILL_DIR}/templates/styles/academic-research/`. Install only that Style
   workspace as the deterministic academic baseline; do not read it before
   Stage 1, do not treat it as a user selection, do not add another confirmation,
   and retain flat free-design packaging. In this branch `free_design` means no
   user-selected reusable layout, not “no academic method/style baseline”. Then
   bind the completed state:
...
> **Default — bounded multimodal web thumbnail selection**: when the current agent can inspect images, add `--save-candidates` to the single or batch web command. Author explicit `query_variants` for materially different official translations, spellings, aliases, or Chinese names; the tool aggregates and deduplicates them, then saves only the first ranked page (8 previews by default), writes `candidates/<stem>/review_sheet.jpg`, marks the batch row `Needs-Selection`, and downloads no original. Review the sheet against the locked Reference/Crop Policy; only a passing `--promote` choice may download one original and write provenance (pass the same `--batch images/image_queries.json` to reconcile its row to `Sourced`). If none passes and `has_more_candidates` is true, advance that row to `next_candidate_page` before changing the query. Only after the pool is exhausted may the row receive materially different query variants and return to `Pending`. A non-multimodal agent omits `--save-candidates`: best-only mode may download only a strict metadata-verified candidate, records `selection_method: metadata-ranked`, and otherwise stops at `Needs-Manual` without claiming visual confirmation.
...
```
Retain references/academic-core.md               # REQUIRED academic/evidence/design overlay
...
|---|---|
| Academic report type was resolved during planning | Retain the already-read `academic-report-types.md` section while realizing §IX; do not re-outline pages during execution |
...
For an academic deck, include the resolved report type, duration, citation
format, and evidence-status policy in this compact confirmation. During SVG
authoring keep page-local short citations, scenario/demo labels, statistical
qualifiers, figure/table captions, and the typography floors from
`academic-core.md`; a passing geometry checker never authorizes their omission.
