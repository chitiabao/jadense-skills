---
description: Validate and install template workspaces, including the deterministic bundled academic Style baseline.
---

# Apply Template Workspace Stage

> Run from [`generate-pptx.md`](../generate-pptx.md) Step 4 after Stage 1 confirms exact workspaces or the academic `free_design` branch below. [`quick-generate`](../profiles/quick-generate.md) enters only for exact roots or a current Create Template handoff. Never load for ordinary free design, bare names, or style descriptions. This stage installs; it does not choose workspaces, alter confirmation, or make the academic baseline a user selection.

## 1. Gate and Normalize Inputs

🚧 **GATE**: Enter through exactly one of these authorities:

1. Default Stage 1 confirmed a non-free template selection.
2. Quick received exact roots directly from the user/current Create Template handoff.
3. Default Stage 1 confirmed academic `free_design`, with no supplied root or explicit Style/template request, and the sole install root is `${SKILL_DIR}/templates/styles/academic-research/`. Preserve the `free_design` receipt, add no confirmation, and retain flat packaging; this baseline is not a user selection.

Any other `free_design` call stops. In Quick, explicit input is the complete selection authority: do
...
The academic baseline reports registered `library` provenance but remains non-selected communication state under Gate branch 3.
...
**Selection cardinality**: Default Stage 1 permits one registered root per kind plus one explicit root; its explicit root may pair with a same-kind library root under §5.2. Gate branch 3 instead permits exactly the single bundled `academic-research` Style root and cannot compose it with any supplied or selected root. Quick has no page or catalog selection and accepts at most one supplied exact root per declared kind (four roots total). Kinds compose freely in both profiles. Reject larger default receipts server-side; require an oversized or duplicate-kind Quick input to converge in chat before installation, never through Confirm UI.
...
```bash
python3 ${SKILL_DIR}/scripts/svg_quality_checker.py "<workspace_root>/templates" --template-mode
```
...
```markdown
> **Installed from**: `${SKILL_DIR}/templates/brands/mckinsey/` (library)
```
