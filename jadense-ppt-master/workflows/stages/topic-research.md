...
> project import.
> Retain the active [`academic-core.md`](../../references/academic-core.md)
> contract. For an academic report, use the selected section of
> [`academic-report-types.md`](../../references/academic-report-types.md) only
> to define required evidence gaps—not to manufacture a standard outline.
...
**Academic boundary**：do not invent or autocomplete an author, paper title,
venue, year, DOI, sample size, statistic, significance result, dataset version,
policy clause, ethics approval, figure identity, or license. If a required item
cannot be verified, retain it as an unresolved gap. Distinguish a source's
reported result from the researcher's interpretation and from the active
presenter's inference.
...
For academic gaps, refine the priority as follows:

| Need | Preferred evidence |
|---|---|
| Paper claim/method/result | Original paper on the publisher/proceedings/repository page; use DOI or another stable landing URL |
| Dataset/software | Official dataset card, repository, release, documentation, or archival identifier |
| Standard/policy/ethics | Issuing body's current official text and exact clause/version |
| Clinical or registered study | Primary publication plus official registry/record when material |
| Correction status | Publisher/repository correction, retraction, or expression-of-concern record |
| Literature positioning | Several directly relevant primary papers; a review may orient but must not impersonate the original evidence |

Read enough of the primary source to support the exact proposed wording; title
and abstract snippets alone do not establish a detailed method/result. Preserve
population/task, comparator, metric, unit, sample, time window, uncertainty and
conditions when they bound the claim. Never infer “significant” from visual
separation or write causation from correlation. If current correction/retraction
status materially affects use, record it as a conflict/qualification.
...
For an academic task, each retained fact in the Markdown states whether it is a
source-reported observation/result, a definition/method fact, or an explicitly
identified inference. Keep bibliographic identity precise enough for later
page-short-citation and notes-full-citation construction. Prefer a DOI/stable
record URL over a search result or temporary viewer link.
...
One `fact_id` must express one citable claim at the source's actual scope. Do
not combine results from several papers into a synthetic claim under one ID,
and do not strip a material qualifier to make the sentence presentation-ready.
Demo/simulation values remain outside `facts` and later receive visible
`Data class: scenario`/`演示数据` treatment.
