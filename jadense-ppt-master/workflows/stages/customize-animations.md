...
Run
`python3 ${SKILL_DIR}/scripts/pptx_animations.py --describe-transition <effect>`
...
`python3 ${SKILL_DIR}/scripts/pptx_animations.py --list` for the exact
...
reference `${SKILL_DIR}/templates/sounds/` from `animations.json`. The
...
`python3 ${SKILL_DIR}/scripts/pptx_animations.py --describe
...
```bash
python3 ${SKILL_DIR}/scripts/video_motion_plan.py \
