#!/usr/bin/env python3
"""
Jadense PPT Master - Prompt Budget Audit Tool

Audits this Skill's agent-facing documents without modifying them. Reports
...
Usage:
    ...
    python3 scripts/prompt_audit.py --root /path/to/jadense-ppt-master

Examples:
    python3 /path/to/jadense-ppt-master/scripts/prompt_audit.py
    python3 /path/to/jadense-ppt-master/scripts/prompt_audit.py --json
...
def audit_schema_grammars(
    ...
    for config in configs:
        ...
        scan_patterns = config.get("scan", ["**/*.md"])
        accepted_by_field = {
...
def render_text(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Jadense PPT Master Prompt Audit",
        "===============================",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Audit Jadense PPT Master's self-contained prompt budget and "
            "governance metadata without writes."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    default_root = _SKILL_DIR
    default_manifest = Path(__file__).with_name("prompt_audit_manifest.json")
    parser.add_argument(
        "--root",
        type=Path,
        default=default_root,
        help=f"Skill root (default: {default_root})",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
