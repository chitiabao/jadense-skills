...
# Skill root directory. PROJECT_ROOT remains as a compatibility alias for
# upstream callers, while all bundled authorities resolve from this Skill.
...
CANVAS_FORMATS = {
    'ppt169': {
        ...
        'height': 720,
        'aspect_ratio': '16:9',
        'use_case': '学术报告默认；现代教室投影、会议报告与在线答辩'
    },
    'ppt43': {
        'name': 'PPT 4:3（旧教室兼容）',
        'dimensions': '1024×768',
        ...
        'height': 768,
        'aspect_ratio': '4:3',
        'use_case': '仅在用户明确要求兼容旧教室或传统投影设备时使用'
    },
    'wechat': {
...
FONTS = {
    'system_ui': "'Microsoft YaHei', DengXian, Arial, 'Segoe UI', sans-serif",
    'sans_serif': "'Microsoft YaHei', DengXian, Arial, 'PingFang SC', sans-serif",
    'academic_zh': "'Microsoft YaHei', DengXian, SimHei, Arial, sans-serif",
...
# config consumers.
SVG_CONSTRAINTS = {
    'authority': (REFERENCES_DIR / 'shared-standards.md').relative_to(SKILL_DIR).as_posix(),
    'core_authority': (REFERENCES_DIR / 'shared-standards-core.md').relative_to(SKILL_DIR).as_posix(),
    'conditional_authorities': {
        'effects': (REFERENCES_DIR / 'svg-effects.md').relative_to(SKILL_DIR).as_posix(),
        'native_data': (REFERENCES_DIR / 'native-data-interface.md').relative_to(SKILL_DIR).as_posix(),
        'pptx_structure': (REFERENCES_DIR / 'pptx-structure-interface.md').relative_to(SKILL_DIR).as_posix(),
    },
    'validator': (SCRIPTS_DIR / 'svg_quality_checker.py').relative_to(SKILL_DIR).as_posix(),
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Jadense 学术 PPT Master 配置管理工具。",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("list-formats", help="列出全部画布格式")
    subparsers.add_parser("list-colors", help="列出全部配色方案")
    subparsers.add_parser("list-industries", help="列出全部行业配色")
