...
def detached_health_pid_matches(launcher_pid: int, reported_pid: object) -> bool:
    """Accept the detached server PID reported by a verified health endpoint.

    Most Python installations keep the PID returned by ``Popen``.  Some
    Windows virtual environments use ``py.exe`` as a launcher, however, so the
    long-running interpreter has a different PID.  Callers still verify the
    service name, project path, port, and launcher liveness; on Windows we also
    require the reported child PID to be alive before accepting that hand-off.
    """
    if isinstance(reported_pid, bool):
        return False
    try:
        reported_pid_int = int(reported_pid)
    except (TypeError, ValueError):
        return False
    if reported_pid_int == launcher_pid:
        return True
    return os.name == 'nt' and process_alive(reported_pid_int)
