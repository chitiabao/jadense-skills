...
**Jadense academic overlay**：本角色始终在
[`academic-core.md`](./academic-core.md) 约束下工作。识别到开题、中期、预答辩、
正式答辩、学术会议、组会、项目/基金评审或其他学术报告时，再读取
[`academic-report-types.md`](./academic-report-types.md) 的对应类型并保留到 §IX 完成。
形式正式、证据可追溯和结论边界优先于艺术性；本覆盖层不改变两阶段确认、Gate、
Design Spec/lock schema 或角色交接。
...
When the task is academic, resolve report type, discipline/topic, degree or
career stage, institution/conference, audience, duration, mandatory sections,
citation format, and evidence status from the request, source, and mandatory
template. Use simplified Chinese for user-facing recommendations when the
primary language is Chinese; retain English program keys, enum ids, schema
fields, formulas, and standard abbreviations.

Default writes these values to `design_spec.md §I Project Information`; Quick
keeps them only in active context; Beautify preserves its frozen source text and
uses the values only for layout/evidence QA. Do not fabricate an institution,
degree stage, conference, duration, citation style, or ethics state. When a
missing value materially changes required sections or evidence treatment,
Default folds it into one existing clarifier; it never creates a new
confirmation stage or unofficial Confirm UI field.

Before authoring §IX, classify each material claim/value as sourced external,
user-supplied, derived, scenario/demo, or unresolved. The outline must carry
page-local short citations, visible demo/scenario disclosure, statistical
qualifiers, and an evidence/limitations duty wherever applicable. A report-type
outline is a reasoning obligation, not permission to add unsupported content.
...
- Academic task with no explicit Style/template → after Stage 1 confirms the
  free-design handoff, the Generate workflow deterministically installs the
  `academic-research` **Style** workspace before Stage 2. Apply that installed
  Style as the default method; do not present it as a Stage-1 user selection or
  read it while authoring Stage 1. This is independent from
  the current `visual_style` axis: author a project-specific `custom` aesthetic
  using only `swiss-minimal`, `editorial`, `data-journalism`, or `blueprint` as
  catalog bases when they serve the discipline and evidence carrier. Never
  write `academic-research` as `visual_style` or `visual_style_references`; it
  is a Style id, not an id in `visual-styles/_index.md`. Art-poster, luxury,
  cartoon, neon, Y2K, collage, marketing-card, or other expressive identities
  require explicit user intent; never propose one merely to make the three
  directions look different.
...
**Academic default**：若用户、院校模板或内容没有明确的语义图标需求，选择
`none`，不要把“专业”误解为每页配图标。优先使用文字、原生形状、公式、图表和真实
科研图像表达关系；院校/机构标识仅在有权使用的真实资产中选取。只有一个紧凑图标
确实降低识别成本时，才选择同一套克制的库。
...
> 4. Put known basenames in the final batch. For an uncertain one, search the chosen style library — or `simple-icons` for a real brand mark — with `rg --files "${SKILL_DIR}/templates/icons/<library>" -g '*<keyword>*.svg'`; do not enumerate broad keyword families.
> 5. **Copy and validate in one batch** — run `python3 ${SKILL_DIR}/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This both validates and materializes `<project>/icons/<lib>/`; skip per-file prechecks.
...
For an academic `1280×720` deck without a mandatory-template override, treat
approximately `68px` cover title, `48px` minimum page title, `24px` minimum
body, and `16–18px` source/footnote as projection-readable floors. When the
planned content does not fit, repair wording/structure/page count within the
confirmed boundaries instead of shrinking below these floors. Use an installed
Chinese face such as Microsoft YaHei/DengXian, Arial for Latin text, and
Cambria Math for Office Math unless the user/template locks another compatible
family.
...
**Academic source order**：default to `none` when charts, tables, formulas and
native SVG can carry the argument. Otherwise prefer `provided` evidence assets,
then rights-compatible `web` figures from an authoritative source. Use `ai`
only for a non-evidentiary conceptual illustration explicitly labeled on-slide
as `概念示意（AI 生成）`; never use AI to synthesize apparent experiments,
microscopy, medical imaging, gels, instrument outputs, fieldwork, survey scenes,
data plots, or paper screenshots. No academic page receives a decorative image
or image quota merely because a visual source is available.
...
```bash
python3 ${SKILL_DIR}/scripts/visualization_recall.py recall \
  --page P03 \
  --tag "time series" \
  --tag "three metrics" \
...
For an academic deck with enabled ordinary notes and no Narration Audio, §X
requires every page note to end in the complete `[Sources]` block defined by
`academic-core.md`. It records only sources actually used on that page, plus an
explicit user-material/demo/no-external-source entry where applicable. When
Narration Audio is enabled, notes remain pure spoken prose for TTS and complete
citations move to page/appendix/source artifacts under the core's audio
boundary; do not make URLs part of the narration merely to satisfy attribution.
...
| Academic project facts, when applicable | §I records report type, discipline, degree/career stage, institution/conference, audience, duration, mandatory sections, citation format, and evidence status; §IX realizes the selected report-type duties without fabricating unknown values |
