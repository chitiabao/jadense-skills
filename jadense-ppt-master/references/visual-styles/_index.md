...
> **Jadense 学术硬门**：学术请求只能自动选择 `swiss-minimal`、`editorial`、`data-journalism` 或 `blueprint`。四者分别服务于通用正式报告、文献与论证密集页面、数据密集页面、工程/系统路线图；默认从 `swiss-minimal` 开始。其余视觉风格完整保留，但只有用户明确点名风格，或明确要求对应的艺术/商业表现时才能启用。诸如“科技”“高端”“年轻”“中国风”等宽泛形容词不足以绕过此门。
>
> 学术默认禁止渐变、3D、玻璃拟态、强投影、营销式卡片阵列、装饰性满版图和花哨动画。即使用户显式选择其他风格，证据真实性、图表可读性、来源和院校模板仍有更高优先级。
...
| 学术任务信号 | 推荐 style | 可选替代 |
|---|---|---|
| 开题、预答辩、正式答辩、基金评审；正式性与论证层级优先 | `swiss-minimal` | `editorial` |
| 相关工作、理论框架、长文本论证、政策或规范分析 | `editorial` | `swiss-minimal` |
| 多指标实验、基线/消融、调查统计、数据报告 | `data-journalism` | `swiss-minimal` |
| 工程、系统架构、技术路线、实验装置或机制图 | `blueprint` | `swiss-minimal` |
| 一份报告同时包含以上任务 | 按主体任务选择一个；通常 `swiss-minimal` | 在单页借用其他三者的局部技法，不改变 deck-wide id |

> 默认 `image_usage` 为 `none`，存在用户提供的科研证据图片时优先 `provided`。只有用户显式启用 AI 图片后，才按配对列解析 rendering；视觉风格本身不得触发 AI 图片。
>
...
商业、产品、摄影、表现性印刷、手绘、水墨、黑板和像素风仍按 §1 的原始 ID 提供。只有用户明确说出该 ID，或明确要求对应表现（例如“使用玻璃拟态”“采用水墨风”）时才读取其明细文件；不得由行业关键词、学术主题名称或“更有设计感”自动推断。显式选择也不得把生成图伪装成实验或观测证据。
