...
> Academic decks retain [`academic-core.md`](./academic-core.md); this branch
> preserves its evidence and citation requirements at cell-grid level.
...
### 2.1 Academic table integrity

- Preserve units, denominators, sample/analysis unit, uncertainty notation,
  significance legend, data version and source footnotes supplied by the
  authority. Right-align comparable numbers and use one declared precision;
  formatting may round only when the underlying value and rule remain clear.
- Keep `0`, `—`, `NA`, `NR`/“未报告”, and “不适用” semantically distinct.
  Never fill a blank from neighboring rows or treat absence of evidence as a
  negative result.
- Literature matrices may include only comparison dimensions explicitly
  supported by the cited papers. Separate paper-reported facts from the
  presenter's interpretation; do not infer an unreported capability from a
  method description.
- Scenario/demo rows or cells carry a visible `演示数据`/`情景数据` label and
  cannot visually masquerade as empirical results. Every external table has a
  nearby short source and, when notes are enabled, a complete page source block.
- Prefer sparse rules, restrained header fill, and readable row height. Do not
  convert a semantic table into disconnected marketing cards or hide qualifiers
  to make it appear cleaner.
