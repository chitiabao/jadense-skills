...
> For an academic project, this rubric also enforces the visible evidence and
> formal-design supplement below; it does not replace factual source review.
...
### §0.2 Academic supplement activation

Activate this supplement when `design_spec.md §I` records an academic report
type/evidence status or the supplied page brief is explicitly an academic
report. A mandatory institution/conference template may override compatible
style and size defaults, but not evidence disclosure. Run these checks after H9
and before Soft rules; record them in the existing JSON schema.

| Rule | Trigger | Disposition |
|---|---|---|
| AH1 | A short citation, source line, demo/scenario label, sample/unit/uncertainty qualifier, figure/table caption, or limitation explicitly required by §IX is absent | content change forbidden here → `needs_human_items`; do not invent or add text |
| AH2 | A required academic disclosure exists but is clipped, overlapped, detached from its object, or unreadable at rendered size; absent template override, source/footnote below 16px or body below 24px is a risk | repair only with ordinary H1–H4/S1–S4 atomic geometry within declared role anchor; otherwise `needs_human_items` |
| AH3 | An AI-generated academic visual is present without a nearby visible `概念示意（AI 生成）` disclosure, or it visually presents itself as experimental/data evidence | `needs_human_items`; never manufacture evidence, caption meaning, or replacement pixels |
| AH4 | A chart/table visibly relies on 3D distortion, undisclosed axis truncation, color alone, or a legend/label layout that prevents accurate comparison against the planned evidence | geometry-only repair when semantics stay intact; otherwise `needs_human_items` |
| AS1 | Decorative full-bleed image, gradient/3D ornament, marketing-card wall, icon repetition, heavy shadow, or animation-like flourish competes with the academic claim/evidence without being locked by user/template | Soft; reduce only unlocked decoration using an existing page token, never delete content or evidence |
| AS2 | Chinese punctuation begins a line, a short title is orphaned, an English term breaks arbitrarily, or citation markers detach from the claim | Soft; reflow within the existing copy and role anchor; never rewrite wording |

The reviewer cannot determine that a paper, DOI, statistic, or significance
claim is true from pixels. It checks only consistency with the supplied plan and
visible disclosure; suspected factual problems are reported as `needs_human`
without proposing a made-up correction.
