...
> Academic decks retain [`academic-core.md`](./academic-core.md); the rules
> below add chart-specific realization without weakening its evidence contract.
...
### 1.1 Academic chart integrity

For an academic page, the authoritative source/active plan must determine the
sample or analysis unit, category definitions, units, domain/baseline, point
estimate, uncertainty, and statistical annotation. Preserve every supplied
qualifier that changes interpretation.

- Put the metric/unit on the axis or direct label; retain `n`, time window,
  dataset/version, random-seed/repetition count, and error-bar definition when
  supplied. Never infer a missing standard deviation, standard error, confidence
  interval, or significance mark.
- Use “显著” only when the source provides the test and result; carry the
  relevant statistic/`p` value/effect size/CI into the page or notes as planned.
  Otherwise use neutral wording such as “观察到差异” or “演示趋势”.
- Scenario, simulation, synthetic, placeholder, or demo values receive a
  nearby visible `演示数据`/`情景数据` label; they must not share an unlabeled
  series with empirical results.
- Keep a page-local short citation/source line tied to the chart and a full
  note source under `executor-notes.md` when enabled. A citation must support the
  actual values and scope, not merely the topic.
- Default to flat 2D encodings, direct labels, restrained gridlines, and
  redundant shape/line/label cues. No 3D, pictorial area distortion, decorative
  gradients, undisclosed truncated axes, or ambiguous dual axes.
- If the source lacks a value, write a declared missing state or return
  upstream; never interpolate or draw a plausible mark to complete the shape.
