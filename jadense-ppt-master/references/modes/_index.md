...
> **Jadense 学术默认**：学术报告以“可复核的建设性顺序 + 结论可追溯”为先，通常解析为基于 `instructional` 与 `pyramid` 的一个 `custom` mode；报告类型不同则按 §2 选择。`narrative` 和 `showcase` 在学术请求中不得自动选择，只有用户明确要求故事弧或展演节奏时才能使用，而且不能改变事实、证据、局限和来源。
...
## 2. 学术自动选择 — 报告类型与评议任务 → mode

| 报告类型 / 评议任务 | 推荐 mode | 可选替代 |
|---|---|---|
| 开题：从问题、缺口、RQ/假设推进到方法、实验设计、风险和计划 | `custom`（`instructional` + `pyramid`） | `instructional` |
| 中期：交代目标、已完成证据、偏差、问题、调整与下一里程碑 | `custom`（`briefing` + `pyramid`） | `briefing` |
| 预答辩 / 正式答辩：先回答核心问题，再用方法和结果闭环支撑贡献 | `pyramid` | `custom`（`pyramid` + `instructional`） |
| 学术会议：有限时间内建立问题、方法、核心结果、局限与结论 | `pyramid` | `instructional` |
| 组会 / 项目汇报：完整呈现进度、异常、待决问题和下一步 | `briefing` | `instructional` |
| 基金 / 项目评审：突出科学问题、创新性、可行性、基础与风险 | `pyramid` | `custom`（`pyramid` + `instructional`） |

> 不能只凭关键词决定 mode。必须同时读取报告类型、`communication_intent`、听众希望得到的结果、核心问题、报告时长、强制章节、交付场景、材料状态和用户给定页序。院校或会议的固定页序优先，mode 只调整页内论证，不得私自删改强制章节。
...
> “keynote-style” 是 mode 请求而不是视觉风格。学术项目只有在用户明确要求时才可采用 `showcase` 节奏；仍需保留研究问题、证据、方法、局限和来源，不得以大图或逐项揭示替代论证。
