...
> Academic decks retain [`academic-core.md`](./academic-core.md). AI output is
> non-evidentiary by default; scientific accuracy language never upgrades it to
> a real observation or result.
...
### 1.1 Academic generation boundary

For an academic deck, use this path only when Strategist/Quick has already
approved a non-evidentiary conceptual illustration. Native SVG, Office Math,
charts, tables, or a sourced real image are preferred whenever exact labels,
measurements, evidence, apparatus identity, anatomy, or reproducibility matter.

**Forbidden academic simulation**：do not generate or edit pixels to resemble
experimental results, microscopy/electron microscopy, medical imaging, gels,
blots, spectra, instrument output, field observations, participant/survey
scenes, maps, data plots, paper screenshots, or a real laboratory/apparatus
photograph. Do not put invented numbers, error bars, scale bars, p-values,
citations, institutional marks, or ethics identifiers into an AI image.

Every approved academic AI item states `non-evidentiary conceptual
illustration` in its purpose/reference, keeps authoritative text/data in SVG,
and requires a visible page caption `概念示意（AI 生成）`. The disclosure is an
SVG-owned caption, not raster lettering. A generated file without that planned
disclosure is not ready for placement even if the image itself exists.
...
**Academic override**：the depth above supports a clearly labeled conceptual
schematic; it does not authorize generating a paper figure or evidentiary image.
If the requested graphic needs exact axes, values, labels, scale bars, chemical
structures, clinical/anatomical fidelity, engineering dimensions, or a
reproducible pipeline, return it to native SVG/shape/chart/table/formula
authoring or use a traceable supplied/source figure. Never fabricate missing
domain detail to make the prompt complete.
...
For academic AI concepts, the core's evidence rule is stricter than this
general positive-trigger table: default to `text_policy: none` and place exact
module names, variables, units, panel labels, citations and disclosure in SVG.
Use `embedded` only for a stable non-evidentiary visual identifier explicitly
approved by the plan; never for data values, statistical marks, axes, scale
bars, source citations, or the `概念示意（AI 生成）` disclosure.
...
- First `.env` found in this order: current working directory, skill directory (e.g. `${SKILL_DIR}/.env`), clone repo root, `~/.ppt-master/.env`
...
- Presenting an AI-generated academic visual as empirical/source evidence, or
  omitting the required visible `概念示意（AI 生成）` disclosure
- Generating apparent research measurements, plots, microscopy, medical scans,
  gels, spectra, instrument output, fieldwork, participant data, paper
  screenshots, scale bars, significance marks, or citations
