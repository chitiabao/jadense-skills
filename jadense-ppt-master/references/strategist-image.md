...
> Retain the always-loaded [`academic-core.md`](./academic-core.md); its
> evidence-image rules override compatible aesthetic preferences for academic
> decks without changing this module's resource schema.
...
**Academic source judgment**：for an academic deck, begin with `none`, then
use provided/extracted evidence assets when they materially support the claim,
then rights-compatible authoritative web figures. AI is limited to a
non-evidentiary conceptual illustration. Never add a decorative hero, spot
family, laboratory scene, paper figure, data plot, microscopy, medical image,
gel, instrument output, or fieldwork image merely to make the deck feel visual.
A real evidence image keeps its caption, scale bar, labels, source and
`no-crop` needs; an AI concept requires a visible SVG caption
`概念示意（AI 生成）` in the owning page plan.
...
For academic work, subject-domain prompt depth never turns generated pixels
into evidence. Prefer native SVG/Office Math/chart/table authoring for exact
scientific schematics, axes, data and labels. If the approved plan still uses AI
for a conceptual mechanism, keep it free of apparent measurements/results, set
its Reference to `non-evidentiary conceptual illustration`, require the
on-slide AI disclosure, and keep authoritative labels/captions in SVG.
...
Academic rows also retain evidence role in `Purpose`/`Reference`: real evidence
names the source/citation and permission requirement; reused paper figures keep
their figure identity; an AI row states that it is conceptual and names the
required on-slide disclosure. A missing citation or license stays
`Pending`/`Needs-Manual`; file availability alone never makes it usable evidence.
