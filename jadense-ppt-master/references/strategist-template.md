...
**Trigger**: Load only after Stage 1 and the post-confirmation apply stage have
made a project-local workspace active: either a confirmed library/explicit
selection, or the Generate workflow's deterministic `academic-research` Style
baseline for an academic free-design request. Bare template names and style
words do not trigger this module. Ordinary non-academic free-design projects do
not trigger it; academic free design does only after the bundled baseline is
actually installed.
